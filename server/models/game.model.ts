...
gameSchema.pre('save', function (next) {
  ...
  if (this.timeEnded && this.timeStarted) {
    this.duration = Math.floor(
      (this.timeEnded.getTime() - this.timeStarted.getTime()) / 1000
    );
  }
...
gameSchema.methods.useHint = function (): {
  ...
  const unguessedLetters = this.word
    .split('')
    .filter((letter: string) => !this.correctLetters.includes(letter));
...
gameSchema.statics.findByUserId = function (userId: string, status?: string) {
  const query: { userId: string; gameStatus?: string } = { userId };
  if (status) {
  ...
  return this.aggregate([
    { $match: { userId: new mongoose.Types.ObjectId(userId) } },
    {
      $group: {
        ...
        totalLosses: {
          $sum: { $cond: [{ $eq: ['$gameStatus', 'lost'] }, 1, 0] },
        },
        totalScore: { $sum: '$score' },
        totalWordsGuessed: {
          $sum: { $cond: [{ $eq: ['$gameStatus', 'won'] }, 1, 0] },
        },
        totalLettersGuessed: { $sum: '$guessesUsed' },
        perfectGames: { $sum: { $cond: ['$perfectGame', 1, 0] } },
        longestWordGuessed: {
          $max: { $cond: [{ $eq: ['$gameStatus', 'won'] }, '$wordLength', 0] },
        },
